...
public class RecordInMemoryStorage implements
		RecordStorageGateway {
	private Map<String, Map<String, DataGroup>> records = new HashMap<>();

	public RecordInMemoryStorage(
	...
	public DataGroup read(String recordType, String recordId) {
		if (null == records.get(recordType)) {
			throw new RecordNotFoundException("No records exists with recordType: "
					+ recordType);
		}
		if (null == records.get(recordType).get(recordId)) {
			throw new RecordNotFoundException("No records exists with recordId: "
					+ recordId);
		}
